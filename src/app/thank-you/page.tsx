...
const ThankYouPage = async ({ searchParams }: IThankYouPageProps) => {
  ...
  if (orderUserId !== user?.id) {
    return redirect(`/sign-in?origin=thak-you?orderId=${order.id}`);
  }
        ...
        <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8 lg:py-32 xl:gap-x-24">
          <div className="lg:col-start-2">
            ...
            {order._isPaid ? (
              ...
              <p className="mt-2 text-base text-muted-foreground">
                We appreciate your order, and w&apos;re currently processing it.
                So hang tight and w&apos;ll send you confirmation very soon!
              </p>
            )}
            <div className="mt-16 text-sm font-medium">
              <div className="text-muted-foreground">Order nr.</div>
              <div className="mt-2 text-gray-900">{order.id}</div>
              <ul className="mt-6 devide-y divide-gray-200 border-t border-gray-200 text-sm font-medium text-muted-foreground">
